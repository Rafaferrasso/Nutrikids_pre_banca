...
// Função para limpar APENAS comentários (executada uma vez na inicialização)
const limparComentariosInicializacao = async () => {
  try {
    console.log('🧹 Limpando comentários da inicialização anterior...');
    
    // Limpar comentários
    const [resultComentarios] = await pool.execute('DELETE FROM comments');
    console.log(`✅ ${resultComentarios.affectedRows} comentários foram apagados na inicialização`);
    
  } catch (error) {
    console.error('❌ Erro ao limpar comentários na inicialização:', error);
  }
};
...
// Função para limpar APENAS curtidas e comentários (preserva usuários)
const limparCurtidasComentarios = async () => {
  try {
    console.log('🧹 Limpando curtidas e comentários...');
    
    // Limpar curtidas
    const [resultCurtidas] = await pool.execute('DELETE FROM likes');
    console.log(`✅ ${resultCurtidas.affectedRows} curtidas foram apagadas`);
    
    // Limpar comentários
    const [resultComentarios] = await pool.execute('DELETE FROM comments');
    console.log(`✅ ${resultComentarios.affectedRows} comentários foram apagados`);
    
    console.log('🎉 Limpeza de curtidas e comentários concluída!');
  } catch (error) {
    console.error('❌ Erro ao limpar curtidas e comentários:', error);
  }
};

// Função para limpar TODOS os dados (incluindo usuários)
const limparDados = async () => {
  try {
    ...
    // Limpar usuários
    const [resultUsuarios] = await pool.execute('DELETE FROM users');
    console.log(`✅ ${resultUsuarios.affectedRows} usuários foram apagados`);
...
process.on('SIGINT', async () => {
  ...
  await limparDados(); // Limpeza completa ao fechar
